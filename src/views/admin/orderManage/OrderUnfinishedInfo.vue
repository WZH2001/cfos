...
export default {
  ...
  methods: {
    load() {
      request
        .get("/order/orderUnfinishedInfo", {
          params: this.params,
        })
        .then((res) => {
          if (res.code === "A0000") {
            this.tableData = res.data.orderUnfinishedInfo;
    ...
    fuzzyQuery() {
      request
        .get("/order/orderUnfinishedInfoFuzzy", {
          params: this.params,
        })
        .then((res) => {
          if (res.code === "A0000") {
            this.tableData = res.data.orderUnfinishedInfoFuzzy;
            this.total = res.data.total;
          } else if (res.code === "A0004") {
            this.$notify.error("服务器异常！");
          }
