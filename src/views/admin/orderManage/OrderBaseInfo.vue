  ...
  <div>
    ...
    <div style="margin-bottom: 20px">
      ...
      <el-button
        ...
        type="success"
        @click="$router.push('/orderUnfinishedInfo')"
        >返回</el-button
      >
    </div>
    <!--表格-->
...
export default {
  ...
  methods: {
    load() {
      request
        .get("/order/orderBaseInfo", {
          params: this.params,
        })
        .then((res) => {
          if (res.code === "A0000") {
            this.tableData = res.data.orderBaseInfo;
    ...
    fuzzyQuery() {
      request
        .get("/order/orderBaseInfoFuzzy", {
          params: this.params,
        })
        .then((res) => {
          if (res.code === "A0000") {
            this.tableData = res.data.orderBaseInfoFuzzy;
            this.total = res.data.total;
          } else if (res.code === "A0004") {
            this.$notify.error("服务器异常！");
          }
